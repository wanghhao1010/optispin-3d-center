# ============================================================================== #
# 🛸 OptiSpin 3D 智慧圖檔大數據中心 - [3D 預覽 + 高科技點雲・全量復活完全體]
# ============================================================================== #
...
def fetch_lightweight_assets():
    """🚀 雙軌防護讀取端：保留完整 filesize 以供 3D 畫布逆向還原"""
    try:
        # 完美捞取所有核心栏位
        ...
        for row in raw_list:
            ...
            safe_row = {
                ...
                "filesize": str(fsize_val)  # 保留完整文字流以供前端 3D 渲染
            }
        ...
        if filtered_data:
            for item in filtered_data:
                with st.container():
                    ...
                    raw_file_url = item.get('filesize', '')
                    ...
                    # 🪐 核心修正：取消所有 if 限制，按鈕強制 100% 亮出！
                    view_col1, view_col2, del_col = st.columns([1.2, 1.2, 1])
                    with view_col1:
                        if st.button(f"🛰️ 3D 模型實體", key=f"btn_m_{asset_id}", use_container_width=True):
                            st.session_state[mesh_toggle_key] = not st.session_state.get(mesh_toggle_key, False)
                            st.session_state[pc_toggle_key] = False
                            st.rerun()
                            
                    with view_col2:
                        if st.button(f"🌌 模擬單色點雲", key=f"btn_pc_{asset_id}", use_container_width=True):
                    ...
                    # 🛠️ 雙軌智能解析渲染引擎面
                    with canvas_slot:
                        # 🎯 建立格式分流器：辨識到底是要讀 http 短網址還是還原 Base64
                        if raw_file_url.startswith("http"):
                            render_src = raw_file_url
                            is_base64_mode = False
                        else:
                            render_src = f"data:application/octet-stream;base64,{raw_file_url}" if raw_file_url else ""
                            is_base64_mode = True

                        # 1. 實體圖層
                        if st.session_state.get(mesh_toggle_key, False) and render_src:
                            if is_usdz:
                                if is_base64_mode:
                                    st.warning("🍏 USDZ 格式請使用全新上傳通道，即可解鎖 1:1 官方 AR 投放功能！")
                                else:
                                    st.success("🍏 iOS 原生 AR 投放通路已就緒！")
                                    st.link_button("📱 點擊此處 → 立即啟動 iPhone 官方空間 AR 投放", url=render_src, use_container_width=True, type="primary")
                            else:
                                html_canvas = f"""
                                <script type="module" src="https://ajax.googleapis.com/ajax/libs/model-viewer/3.4.0/model-viewer.min.js"></script>
                                <model-viewer src="{render_src}" alt="OptiSpin" camera-controls auto-rotate style="width: 100%; height: 320px; background-color: #1a1a1a; border-radius: 10px;"></model-viewer>
                                """
                                st.components.v1.html(html_canvas, height=330)

                        # 2. 模擬點雲圖層
                        if st.session_state.get(pc_toggle_key, False) and render_src:
                            if is_usdz:
                                st.warning("🌌 點雲模擬目前專屬於工業 GLB 格式！")
                            else:
                                with st.spinner("🌌 正在精準逆向還原高科技點雲..."):
                                    try:
                                        if is_base64_mode:
                                            file_content = base64.b64decode(raw_file_url)
                                        else:
                                            res_file = requests.get(render_src, timeout=15)
                                            file_content = res_file.content
                                            
                                        scene_or_m = trimesh.load(io.BytesIO(file_content), file_type='glb')
                                        ...
                                        fig = go.Figure(data=[go.Scatter3d(x=pts[:, 0], y=pts[:, 1], z=pts[:, 2], mode='markers', marker=dict(size=2.5, color='#ffffff', opacity=0.85))])
